    ...
    <form method="post">
        ...
        <div class="flex flex-col gap-5 my-5">
            ...
            <div class="form-control">
                ...
                <input type="text" name="member-id" placeholder="Ketikkan ID Member jika tersedia"
                    class="input input-bordered min-w-[22rem]"
                    <?= !empty($transaction) ? 'value="' . $transaction['id_member'] . '"' : "" ?> />
            </div>
            <div class="overflow-x-auto">
                <h4 class="text-sm font-medium mb-2">Detail Keranjang</h4>
                <input type="number" id="total-product-saved" name="total-product-saved"
                    <?= !empty($transaction) ? 'value="' . count($transaction_details) . '"' : "" ?> class="hidden">
                <table class="table w-full">
                    ...
                    <tbody id="transaction-detail-container">
                        ...
                        <tr>
                            <input type="number" name="<?= $index ?>-product-id" value="<?= $detail->id_produk ?>"
                                class="hidden">
                            <input type="number" name="<?= $index ?>-stock-id" value="<?= $detail->id_stok ?>"
                                class="hidden">
                            <input type="number" name="<?= $index ?>-stock-needed-to-buy"
                                value="<?= $detail->jumlah_produk ?>" class="hidden">
                            <input type="number" name="<?= $index ?>-max-stock"
                                value="<?= $detail->stok + $detail->jumlah_produk ?>" class="hidden">
                            <input type="text" name="<?= $index ?>-product-name" value="<?= $detail->nama_baju ?>"
                                class="hidden">
                            <input type="number" name="<?= $index ?>-product-price" value="<?= $detail->harga ?>"
                                class="hidden">
                            <input type="text" name="<?= $index ?>-size" value="<?= $detail->ukuran ?>"
                                class="hidden" />
                            ...
                            <td class="flex gap-1">
                                <button type="button" id="<?= $detail->id_produk ?>-<?= $detail->ukuran ?>-add-action"
                                    class="add-action-btn btn btn-error btn-sm btn-square">+</button>
                                <button type="button" id="<?= $detail->id_produk ?>-<?= $detail->ukuran ?>-minus-action"
                                    class="minus-action-btn btn btn-info btn-sm btn-square">-</button>
                            </td>
                        </tr>
                        <?php $index++ ?>
                        <?php endforeach ?>
        ...
        <div class="flex gap-2">
            <button type="submit" id="finish-transaction-btn" formaction="/transaksi/bayar_transaksi"
                class="hidden transaction-btn btn btn-sm btn-primary">Bayar</button>
            <button type="submit" id="save-transaction-btn" formaction="/transaksi/save_transaksi"
